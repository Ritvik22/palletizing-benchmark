...
from experiment_results import DATASET, CLUSTER_DATASET, ZIP1_DATASET, BEST_KNOWN_DATASET, LATEST_EP_DATASET, ExperimentResults, ClusterExperimentResults, Zip1ExperimentResults, BestKnownExperimentResults, LatestEPExperimentResults, order_revisions, result_router  # noqa: E402
...
EXTRA_EXPERIMENTS = []
for dataset, library_type in ((CLUSTER_DATASET, ClusterExperimentResults), (ZIP1_DATASET, Zip1ExperimentResults), (BEST_KNOWN_DATASET, BestKnownExperimentResults), (LATEST_EP_DATASET, LatestEPExperimentResults)):
    archive_root = HERE / 'experiments' / dataset
    if (archive_root / 'viewer.json').is_file():
        archive_library = library_type(archive_root)
        EXTRA_EXPERIMENTS.append(archive_library)
        app.include_router(result_router(archive_root, archive_library))
